...
function normalizeCompareData(raw: any): CompareData | null {
  ...
  const normalizeDims = (d: any) => ({
    ie: d?.ie ?? { letter: "I", score: 50, evidence: "" },
    ns: d?.ns ?? { letter: "N", score: 50, evidence: "" },
    tf: d?.tf ?? { letter: "T", score: 50, evidence: "" },
    jp: d?.jp ?? { letter: "J", score: 50, evidence: "" },
  });
  ...
  return {
    ...
    personA: {
      ...raw.personA,
      dimensions: normalizeDims(raw.personA?.dimensions),
    ...
    personB: {
      ...raw.personB,
      dimensions: normalizeDims(raw.personB?.dimensions),
    ...
    comparison: {
      ...
      differences: Array.isArray(comparison.differences)
        ...
        : [],
      sharedWorks: Array.isArray(comparison.sharedWorks)
        ? comparison.sharedWorks
        : [],
      crossRecommend,
      matchScore:
        typeof comparison.matchScore === "number" ? comparison.matchScore : 0,
      matchTitle:
        typeof comparison.matchTitle === "string"
          ? comparison.matchTitle
          : "匹配度",
      overview: typeof comparison.overview === "string" ? comparison.overview : "",
      chemistry:
        typeof comparison.chemistry === "string" ? comparison.chemistry : "",
    },
  };
}
    ...
    <main className="min-h-screen px-4 py-8">
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="card-glass rounded-xl p-5 space-y-4 animate-fade-in-up animate-delay-100">
          <h3 className="text-sm font-bold text-[#667eea]">🧬 四维度对比</h3>
          {DIM_KEYS.map((key) => (
            <DualDimensionBar
              key={key}
              dimKey={key}
        ...
        <div className="card-glass rounded-xl p-5 animate-fade-in-up animate-delay-100">
          <h3 className="text-sm font-bold text-[#667eea] mb-3">📊 品味雷达对比</h3>
          <DualRadar
        ...
        <div className="card-glass rounded-xl p-5 space-y-2 animate-fade-in-up animate-delay-200">
          <h3 className="text-sm font-bold text-[#e94560]">💡 匹配解读</h3>
          <p className="text-sm text-gray-300 leading-relaxed">{comparison.overview}</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 animate-fade-in-up animate-delay-200">
          <div className="card-glass rounded-xl p-5 space-y-3">
            <h3 className="text-sm font-bold text-green-400">✅ 相同点</h3>
            <div className="space-y-3">
              {comparison.similarities.map((s, i) => (
                <div key={i} className="space-y-1">
                  <div className="text-xs font-medium text-white">{s.point}</div>
          ...
          <div className="card-glass rounded-xl p-5 space-y-3">
            <h3 className="text-sm font-bold text-[#e94560]">⚡ 不同点</h3>
            <div className="space-y-3">
              {comparison.differences.map((d, i) => (
                <div key={i} className="space-y-1">
                  <div className="text-xs font-medium text-white">{d.point}</div>
        ...
        {/* Chemistry */}
        <div className="card-glass rounded-xl p-5 space-y-2 animate-fade-in-up animate-delay-200">
          <h3 className="text-sm font-bold text-[#f5c518]">⚗️ 化学反应</h3>
          <p className="text-sm text-gray-300 leading-relaxed whitespace-pre-line">{comparison.chemistry}</p>
        </div>
        ...
        {(comparison.roastOneLiner ||
          ...
          <div className="card-glass rounded-xl p-5 space-y-4 animate-fade-in-up animate-delay-200">
            <h3 className="text-sm font-bold text-[#a78bfa]">🎯 趣味彩蛋</h3>
            {comparison.roastOneLiner && (
              <div className="space-y-1">
                <span className="text-[10px] text-gray-500 uppercase tracking-wider">毒舌吐槽</span>
                <p className="text-sm text-gray-200 italic">
                  &ldquo;{comparison.roastOneLiner}&rdquo;
                </p>
              </div>
            )}
            {comparison.dateScene && (
            ...
            {comparison.memeLine && (
              <div className="pt-2 border-t border-white/5">
                ...
                <p className="text-sm text-amber-300/90 mt-1 font-medium">
                  &ldquo;{comparison.memeLine}&rdquo;
                </p>
              </div>
            )}
          </div>
        ...
        {comparison.sharedWorks.length > 0 && (
          <div className="card-glass rounded-xl p-5 space-y-3 animate-fade-in-up animate-delay-200">
            <h3 className="text-sm font-bold text-[#667eea]">🔗 品味交集</h3>
            <p className="text-xs text-gray-500 mb-2">你们都看过/读过/听过的作品</p>
        ...
        {comparison.crossRecommend &&
          (comparison.crossRecommend.forA.length > 0 ||
            ...
            <div className="card-glass rounded-xl p-5 space-y-4 animate-fade-in-up animate-delay-300">
              <h3 className="text-sm font-bold text-[#e94560]">💡 交叉推荐</h3>
        ...
        <div className="card-glass rounded-xl p-5 animate-fade-in-up animate-delay-300">
          <h3 className="text-sm font-bold text-[#667eea] mb-3">📊 数据对比</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="text-xs text-gray-500 text-center font-medium">{personA.name}</div>
            ...
            <div className="space-y-2">
              <div className="text-xs text-gray-500 text-center font-medium">{personB.name}</div>
              <div className="flex justify-center gap-2 text-xs">
        ...
        <div className="text-center space-y-3 pb-8 animate-fade-in-up animate-delay-300">
          ...
          <p className="text-xs text-gray-500">豆瓣书影音 MBTI · 品味即人格</p>
        </div>
      </div>
    </main>
  );
  ...
  return (
    <div className="space-y-4">
      {previewSrc && (
        ...
        >
          <p className="text-white text-sm mb-3 animate-pulse">👆 长按图片保存到相册</p>
        ...
        <div className="relative z-10 p-6">
          ...
          <div className="flex items-center justify-center gap-4 mb-4">
            <div className="text-center">
              <div className="text-2xl font-black tracking-wider mb-0.5" style={{ color: "#667eea" }}>
                {personA.mbtiType}
              </div>
              <div className="text-[10px] text-gray-400">{personA.name}</div>
              <div className="text-[9px] text-gray-500 mt-0.5">{personA.mbtiTitle}</div>
            </div>
            <div className="flex flex-col items-center">
              <div className="text-3xl font-black" style={{ color: matchColor }}>
                {comparison.matchScore}
              </div>
              <div className="text-[8px] text-gray-500 uppercase tracking-wider">match</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-black tracking-wider mb-0.5" style={{ color: "#e94560" }}>
                {personB.mbtiType}
              </div>
              <div className="text-[10px] text-gray-400">{personB.name}</div>
              <div className="text-[9px] text-gray-500 mt-0.5">{personB.mbtiTitle}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-[10px] text-gray-600 border-t border-white/5 pt-3">
            ...
            <span className="text-[#667eea]">测测你们的书影音 MBTI →</span>
          </div>
        </div>
      </div>
...
function DualRadar({
  ...
  const pointsA = RADAR_LABELS.map(([key], i) => getPoint(i, dataA[key] ?? 50));
  const pointsB = RADAR_LABELS.map(([key], i) => getPoint(i, dataB[key] ?? 50));
  const polyA = pointsA.map((p) => `${p.x},${p.y}`).join(" ");
  const polyB = pointsB.map((p) => `${p.x},${p.y}`).join(" ");

  return (
    <div>
      <svg width="100%" viewBox={`0 0 ${size} ${size}`} className="max-w-[200px] mx-auto">
        ...
        <polygon points={polyA} fill="rgba(102,126,234,0.12)" stroke="#667eea" strokeWidth="1.5" />
        <polygon points={polyB} fill="rgba(233,69,96,0.12)" stroke="#e94560" strokeWidth="1.5" />
      </svg>
...
function RecItem({ rec }: { rec: CrossRecItem }) {
  const searchUrl = `https://search.douban.com/${
    rec.type === "book" ? "book" : rec.type === "movie" ? "movie" : "music"
  }/subject_search?search_text=${encodeURIComponent(rec.title)}`;
  return (
    <a
      href={searchUrl}
      target="_blank"
